...
function stochastic_ppc{A <: Algorithm}(dfunc::Function, X, k_means::K_MEANS{A}, alg_params::Vector, 
    ...
    while true
    	...
    	if ~failed_mapping && (mappings_not_changed || t > k_means.max_iter) 
    		break # check cluster mappings has not changed and exit
    	end

    	if t > k_means.max_iter*10
