...
@test_approx_eq_eps mean(ripley["Yt"] .== model.output.Ytest) 0.881 0.01
...
model = salsa(ripley["X"],ripley["Y"],model,ripley["Xt"])
@test_approx_eq_eps mean(ripley["Yt"] .== model.output.Ytest) 0.887 0.01
...
model = salsa(ripley["X"],ripley["Y"],model,ripley["Xt"])
@test_approx_eq_eps mean(ripley["Yt"] .== model.output.Ytest) 0.892 0.01

srand(1234)
model = SALSAModel(LINEAR,R_L1RDA,HINGE,global_opt=DS([-5,0,0,-2]))
model = salsa(ripley["X"],ripley["Y"],model,ripley["Xt"])
@test_approx_eq_eps mean(ripley["Yt"] .== model.output.Ytest) 0.877 0.01
